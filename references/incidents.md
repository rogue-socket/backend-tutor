...
### Saga / outbox pattern (positive case studies)
**Sources:** uber.com/blog "Cadence: The Only Workflow Platform You'll Ever Need" (origin post for Uber's workflow engine, since spun out as Temporal); microservices.io/patterns/data/transactional-outbox.html (Chris Richardson's reference); debezium.io docs (the canonical CDC engine outbox draining is built on); Microservices Patterns ch. 4-5 (Richardson book); Building Microservices 2e ch. 6 (Newman).
**What happened:** Like the T1 Stripe entry, this is a sustained-discipline reference rather than a single-incident RCA — public postmortems naming "we built saga wrong → outage" are scarce because the failure is usually silent data divergence rather than a visible outage. **Saga**: when a logical workflow spans multiple services (order → reserve inventory → charge payment → ship), distributed transactions (2PC) are avoided in modern systems for availability reasons, so the answer is *compensating actions* on partial failure — refund the charge if shipping fails, restock the inventory if the charge fails. Uber's Cadence (now Temporal) exists specifically because hand-rolled saga state machines became unmaintainable at scale; the engineering blog frames the rebuild around "we kept getting workflow correctness wrong by hand and our compensations had bugs." **Outbox**: when a service must both update its DB and publish an event, dual-write is unsafe — the broker can succeed while the DB commit fails, or vice versa, leaving the system inconsistent. The outbox pattern writes the event to an `outbox` table in the *same DB transaction* as the state change, then a separate process (often a CDC tool like Debezium reading the WAL) drains the table to the broker. The receiver pairs this with an inbox table for idempotent consumption. eBay, Shopify, and many others built their event-driven backbones on this.
**Teach with it:** T3.10 (saga + distributed transactions — two-phase commit's failure mode is availability; saga's failure mode is correctness-of-compensations); T11 cross-link (saga is an architecture-level pattern; the *implementation* — outbox table, CDC replicator, idempotent compensations, retry semantics on compensations — is what backend engineers own). Senior-grade discussion: "when do you build vs adopt?" — Cadence/Temporal vs hand-rolled saga state machine.
